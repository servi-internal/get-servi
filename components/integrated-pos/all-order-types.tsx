...
export function AllOrderTypes() {
  return (
    <section id="all-order-types" className="py-10 sm:py-14 lg:py-16 bg-gray-50">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 min-w-0">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-12">
          {ORDER_TYPES.map((type, i) => (
            <div
              key={i}
              className="p-4 sm:p-5 rounded-xl bg-white border border-gray-100 min-w-0"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-8 sm:mb-12">
          {FEATURE_ICONS.map((item, i) => (
            <div
              key={i}
              className="group p-3 sm:p-4 rounded-xl border bg-gradient-to-br from-white to-gray-50 border-gray-100 hover:border-[#FF6600]/30 transition-all min-w-0"
            >
              <div className="flex items-start gap-2 sm:gap-3">
                <div
                  className={`w-9 h-9 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${colorClasses[item.color]}`}
                >
                  <i className={`bi ${item.icon} text-base sm:text-lg`} />
                </div>
                <div className="min-w-0">
                  <h3 className="font-bold text-xs sm:text-sm mb-0.5 text-gray-900">
                    {item.title}
                  </h3>
                  <p className="text-[10px] sm:text-xs leading-snug text-gray-600">
                    {item.desc}
                  </p>
                </div>
              </div>
