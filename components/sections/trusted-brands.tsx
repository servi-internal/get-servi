import Image from "next/image";

export function TrustedBrands() {
  return (
    <section className="w-full bg-white py-12 sm:py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <p className="mb-6 sm:mb-8 text-xs sm:text-sm font-bold uppercase tracking-widest text-gray-400 text-center">
          Trusted By <span className="text-[#FF6600]">Restaurants</span>
        </p>
        <div className="relative w-full overflow-hidden">
          {/* Left gradient fade */}
          <div className="absolute left-0 top-0 bottom-0 w-16 sm:w-32 bg-gradient-to-r from-white to-transparent z-10 pointer-events-none"></div>
          {/* Right gradient fade */}
          <div className="absolute right-0 top-0 bottom-0 w-16 sm:w-32 bg-gradient-to-l from-white to-transparent z-10 pointer-events-none"></div>
          
          <div className="flex animate-scroll-logos">
            <div className="flex items-center gap-8 sm:gap-12 flex-shrink-0">
              <Image src="/logos-webp/rosa.webp" alt="Rosa's Pizzeria" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/midland.webp" alt="Midland Bagels" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/dangelos.webp" alt="D'Angelos" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/piatto.webp" alt="Piatto" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/dolce.webp" alt="Dolce" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/cafe-aroma.webp" alt="Cafe Aroma" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/garviespoint.webp" alt="Garvies Point" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/companatico.webp" alt="Companatico" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/allproplecoffee.webp" alt="All Prople Coffee" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/newriverdale.webp" alt="New Riverdale" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
            </div>
            <div className="flex items-center gap-8 sm:gap-12 flex-shrink-0 ml-8 sm:ml-12">
              <Image src="/logos-webp/rosa.webp" alt="Rosa's Pizzeria" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/midland.webp" alt="Midland Bagels" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/dangelos.webp" alt="D'Angelos" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/piatto.webp" alt="Piatto" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/dolce.webp" alt="Dolce" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/cafe-aroma.webp" alt="Cafe Aroma" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/garviespoint.webp" alt="Garvies Point" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/companatico.webp" alt="Companatico" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/allproplecoffee.webp" alt="All Prople Coffee" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
              <Image src="/logos-webp/newriverdale.webp" alt="New Riverdale" width={240} height={120} className="object-contain h-16 sm:h-20 md:h-24 w-[150px] sm:w-[200px] md:w-[240px] flex-shrink-0 transition-all" />
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}

